...
@pytest.mark.asyncio
async def test_find_user_by_email_not_found(client: AsyncClient) -> None:
    """
    Test the behavior when no user by email are found.

    This test checks that the endpoint returns a 404 status code
    and the correct error message when no user are found.
    """  
    response = await client.get("/users/email/user@example.com")
    assert response.status_code == status.HTTP_404_NOT_FOUND
    assert response.json() == {"detail": "User not found"}
...
@pytest.mark.asyncio
async def test_find_user_by_email(client: AsyncClient) -> None:
    """
    Test the retrieval of user by email.
    ...
    response = await client.get(f"/users/email/{user_email}")
    assert response.status_code == status.HTTP_200_OK
